...
import {
  ...
  FileText, Loader, Ban, DollarSign, UserX, ShieldOff, Unlock, CalendarRange, FileSpreadsheet
} from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-4">
          ...
          <div className="flex items-center gap-1 bg-white border border-slate-200 rounded-xl px-2 py-1 shadow-sm">
            ...
            <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)} max={dateTo || undefined}
              className="text-[11px] text-slate-600 focus:outline-none bg-transparent" title="From date"/>
            <span className="text-slate-300 text-[11px]">–</span>
            <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)} min={dateFrom || undefined}
              className="text-[11px] text-slate-600 focus:outline-none bg-transparent" title="To date"/>
            {(dateFrom || dateTo) && (
              <button onClick={() => { setDateFrom(''); setDateTo(''); }} title="Clear date range">
